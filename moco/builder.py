...
class MoCo(nn.Module):
    ...
    def forward(self, im_q, im_k, label):
        ...
        label_curbatch = concat_all_gather(label) # B
        ...
        label_enqueue = torch.cat([label_curbatch, label_], dim=0)  # B+K


        # logits: Nx(1+K)
        # logits = torch.cat([l_pos, l_neg], dim=1)

        logits = torch.einsum('nc,ck->nk', [q, queue_enqueue])   # Nx(B+K)
        ...
        # labels_queue = self.label_queue.clone().detach()

        # dequeue and enqueue
        self._dequeue_and_enqueue(k, label)

        return logits, label_enqueue
